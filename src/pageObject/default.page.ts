...
//import config from "../resources/json/timeout.json"
export default class DefaultPage {
  ...
  async fillEnvironment() {
    await test.step("ENter  environment details ", async () => {
      ...
      await this.page
        .locator("div")
        .filter({ hasText: /^Application Type\*NoneWebAPI$/ })
        .getByRole("combobox")
        .selectOption("WEB");
      await this.page.getByPlaceholder("example.com", { exact: true }).click();
      await this.page.waitForTimeout(5000);
      await this.page
        .getByPlaceholder("example.com", { exact: true })
        .fill("example.com");
      ...
      await this.page.screenshot({
        path: "./src/resources/snapShots/setttingPage.png",
        fullPage: true,
      });
      await this.page.getByRole("button", { name: "Environment" }).click();
      ...
      await this.page.waitForTimeout(5000);
      await this.page.screenshot({
        path: "./src/resources/snapShots/darkMode.png",
        fullPage: true,
      });
